...
### Key Requirements
- ✅ Web-based solution (NO native mobile app)
- ✅ QR codes generated by admins for each site
- ✅ Agents scan QR codes with smartphone camera
- ✅ Ultra-minimal agent interface (no company info, no schedules, no time data)
- ✅ Records time entries with anti-fraud tracking
- ✅ Separate domain for agent access
- ✅ Simple confirmations: "Clock-in validated" / "Clock-out validated"
...
<div class="container">
  <% if @can_clock_in %>
    ...
    <%= form_with url: agent_clock_in_path(token: params[:token]), method: :post, local: true do |f| %>
      <%= f.submit "Pointer l'entrée", class: "btn btn-primary" %>
    <% end %>
  <% elsif @can_clock_out %>
    <div class="icon">📤</div>
  ...
  <% else %>
    ...
    <p style="margin: 20px 0;">Vous êtes pointé sur un autre site.</p>
  <% end %>
</div>
```
...
<div class="page-header">
  ...
  <%= link_to "← Retour", admin_sites_path, class: "btn btn-secondary" %>
</div>
...
<div class="card" style="max-width: 600px; margin: 0 auto; text-align: center;">
  <div class="card-body">
    ...
    <div style="margin-top: 30px;">
      <%= link_to "🖨️ Version imprimable", print_qr_admin_site_path(@site), 
                  class: "btn btn-primary", 
                  target: "_blank" %>
      <%= link_to "📄 Télécharger PNG", "#", 
...
```erb
<%= link_to "📱 QR Code", qr_code_admin_site_path(site), class: "btn btn-sm btn-info" %>
```
...
namespace :sites do
  ...
  task generate_qr_tokens: :environment do
    ...
    Site.where(qr_code_token: nil).find_each do |site|
      ...
      sites_updated += 1
      puts "✅ Generated QR token for: #{site.name}"
    end
    
    puts "\n🎉 Updated #{sites_updated} site(s)"
  end
end
```
...
- [ ] Success message displays: "✅ Pointage d'entrée validé"
- [ ] Can scan again to clock out
- [ ] Clock-out button works
- [ ] Success message displays: "✅ Pointage de sortie validé"
...
Set up two domains:
- **Agent domain:** `clock.company.com` → Agent QR scanning interface
- **Admin domain:** `admin.company.com` → Admin/Manager interface
...
- ✅ All 100-140 agents can successfully scan QR codes
- ✅ Clock-in/out process takes < 10 seconds
- ✅ Mobile interface works on iOS and Android
- ✅ No company information visible to agents
- ✅ Anti-fraud tracking captures IP and user agent
- ✅ Admins can generate and print QR codes
- ✅ Time entries record correctly in database
- ✅ No duplicate active entries possible
